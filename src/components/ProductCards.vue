        ...
        <div class ="bottom-side">
          ...
          <div>
            ...
            <button class="sizeButton" v-for="(shoeSize,index) in productInfo.priceDueSize" :key="index"   
              ...
              @click="getPriceFromSize(shoeSize); size = shoeSize.size; changeColor()">
          ...
          <div>     
            ...
            Total: ${{calcPrice}} </strong></p>
             <div style="display:flex" id="button--row" v-if="calcPrice>0 && quantity>0">
              ...
              <button class="quantityButton" @click="subtract(); changeColor()"
              :style="{backgroundColor}">
              -</button>
              <p style="margin-bottom:0; padding: 5px" :style="{color}"><strong>{{ quantity }}</strong></p>
              <button class="quantityButton" @click="add(); changeColor()"
              :style="{backgroundColor}">
               + </button>
            </div>
...
methods:{
  ...
  changeColor() {
    return document.getElementsByClassName("sizeButton").style.backgroundColor="whitesmoke";
  } 
